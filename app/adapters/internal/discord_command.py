...
class DiscordCommandAdapter(commands.Cog):
    ...
    @app_commands.command(
        name="dca-status", description="자동매매 진행 상황을 확인합니다"
    )
    ...
    @app_commands.command(
        name="trade-start", description="새로운 자동매매를 시작합니다"
    )
    ...
    @app_commands.command(
        name="trade-stop", description="진행 중인 자동매매를 중단합니다"
    )
        ...
        try:
            ...
            if not dca_list:
                embed = discord.Embed(
                    ...
                    "새로운 DCA를 시작하려면 `/trade-start` 커맨드를 사용하세요.",
                    color=0x808080,
                )
                await interaction.followup.send(embed=embed, ephemeral=True)
                return
